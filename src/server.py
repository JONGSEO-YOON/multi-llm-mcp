...
- 사용자 가이드 작성""",
        ...
        Tool(
            ...
            description="""Multimodal Looker - 이미지 분석 에이전트 (Gemini)
...
async def _check_status() -> list[TextContent]:
    ...
    if has_codex_cli():
        status_lines.append("[O] Codex CLI 설치됨")
        status_lines.append("   -> 'codex' 명령어로 ChatGPT 계정 로그인 가능")
    else:
        status_lines.append("[X] Codex CLI 미설치")
        status_lines.append("   -> 'npm install -g @openai/codex'로 설치")

    if has_openai_api_key():
        status_lines.append("[O] OPENAI_API_KEY 설정됨")
    else:
